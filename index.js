...
function togglemode() {
	if (mode == 'letter') {
		slider.setAttribute('style', `left:${letter.offsetLeft - 12}px;top:${12}px;background-color:#48C9B0; padding:2rem; display:flex;
		align-item:center;`);
		pagestyle(1);
	}
	if (mode == 'assignment') {
		slider.setAttribute('style', `left:${assignment.offsetLeft - 13}px;top:${12}px;background-color: #48C9B0;width:200px;padding:2rem;`);
		pagestyle(1);
	}
}
togglemode();
...
function turnoff() {
	...
	function turnoffmicrophone() {
		turnSwitch = 0;
		recognition.stop();
		document.querySelector('#mic').classList.remove("uil-microphone");
		document.querySelector('#mic').classList.add("uil-microphone-slash");
	...
	function turnonmicrophone() {
		turnSwitch = 1;
		recognition.start();
		document.querySelector('#mic').classList.remove("uil-microphone-slash");
		document.querySelector('#mic').classList.add("uil-microphone");
